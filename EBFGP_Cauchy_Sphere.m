...
% Cauchy spatiotemporal covariance family (in the Gneiting class) restricted 
% to the sphere in R3
% 
%--------------------------------------------------------------------------
...
%% --- STEP 1.1: RESOLUTION AND TRUNCATION PARAMETERS (UNDER SPHERICAL SPARSE OBSERVATION OF FUNCTIONAL DATA) ---

T = 1000;          % Functional sample size (number of observed times)
...
N = 50;            % Number of spherical locations (the minimum threshold required is N=500 to ensure suitable reconstruction when applying inverse spherical harmonic transform) 
                   % The sparsest spherical scenario is attained when N=50
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% STAGE 2: PRIOR COVARIANCE HYPERPARAMETER SAMPLING AND CAUCHY SPATIOTEMPORAL COVARIANCE  SUBFAMILY RESTRICTED TO SPHERE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
A3 = 5; B3 = 7; pgamma = betarnd(A3, B3, M, 1);   
A4 = 2; B4 = 8; nu = betarnd(A4, B4, M, 1);      

% Additive unstructured observation noise variance hyperparameter (contamination parameter)
mu = 1/4;
sigmap = 0.01; 
sigma = normrnd(mu, sigmap, M, 1);

%% --- STEP 2.2: RESTRICTION OF CAUCHY COVARIANCE FUNCTION TO THE SPHERE (TRUE HYPERPARAMETER VECTOR VALUE) ---
...
% 2.2.3 Cauchy spatiotemporal covariance kernel restricted to sphere
psinv = (psi).^(-1);
...
for i = 1:N            
    for j = 1:N
        scnorm = C1(i,j) * psinv;
        CEBA1(i,j,:) = psinv .* (ones(T,1) + c * scnorm.^(2 * pgamma(theta0,1))).^(-nu(theta0,1));
    end
end
...
%% --- STEP 3.4: TIME-VARYING KARHUNEN-LOÈVE-BASED SIMULATION OF FGP---
...
for r = 1:R
    for t = 1:T
        KLRCOREB(:,t,r) = (TEIGC(t,:)').^(1/2) .* randn(TR,1); 
        for j = 1:N
            TORIGEB(:,j,t) = KLRCOREB(:,t,r) .* RR1(j,:)';
        end
        STORIGEB = sum(TORIGEB, 1);
        YORIGEB(:,t,r) = STORIGEB(1,:,t);
    end   
end
...
% Array dimension
CEBA1EB = zeros(N, N, T, M);

% Evaluation of  Cauchy subfamily example of spatiotemporal covariance
...
for theta = 1:M 
    ...
    for i = 1:N            
        for j = 1:N
            ...
            CEBA1EB(i,j,:,theta) = psinv_k .* (ones(T,1) + c * scnorm.^(2 * pgamma(theta,1))).^(-nu(theta,1));
        end
    end
end
...
zzb4 = 1; 
figure('Name', 'Posterior spherical functional mean');
sgtitle('Posterior spherical functional mean')
for t = 1:fix(DELTA):T       
